import { AnimationClip, Bone, Box3, BufferAttribute, BufferGeometry, CanvasTexture, ClampToEdgeWrapping, Color, DirectionalLight, FileLoader, Group, ImageBitmapLoader, InterleavedBuffer, InterleavedBufferAttribute, Interpolant, InterpolateDiscrete, InterpolateLinear, Line, LineBasicMaterial, LineLoop, LineSegments, LinearFilter, LinearMipmapLinearFilter, LinearMipmapNearestFilter, Loader, LoaderUtils, Material, Math as MathUtils, Matrix4, Mesh, MeshBasicMaterial, MeshPhysicalMaterial, MeshStandardMaterial, MirroredRepeatWrapping, NearestFilter, NearestMipmapLinearFilter, NearestMipmapNearestFilter, NumberKeyframeTrack, Object3D, OrthographicCamera, PerspectiveCamera, PointLight, Points, PointsMaterial, PropertyBinding, QuaternionKeyframeTrack, RGBFormat, RepeatWrapping, Skeleton, SkinnedMesh, Sphere, SpotLight, TextureLoader, Vector2, Vector3, VectorKeyframeTrack, sRGBEncoding } from "./three.js";
...
var GLTFLoader = class extends Loader {
	...
	parse(data, path, onLoad, onError) {
		...
		else if (LoaderUtils.decodeText(new Uint8Array(data, 0, 4)) === BINARY_EXTENSION_HEADER_MAGIC) {
			try {
		...
		} else content = LoaderUtils.decodeText(new Uint8Array(data));
...
var GLTFMaterialsUnlitExtension = class {
	...
	extendParams(materialParams, materialDef, parser) {
		...
		if (metallicRoughness) {
			...
			if (metallicRoughness.baseColorTexture !== void 0) pending.push(parser.assignTexture(materialParams, "map", metallicRoughness.baseColorTexture));
		}
	...
	decodePrimitive(primitive, parser) {
		...
		for (const attributeName in primitive.attributes) {
			...
			if (gltfAttributeMap[attributeName] !== void 0) {
				...
				attributeTypeMap[threeAttributeName] = WEBGL_COMPONENT_TYPES[accessorDef.componentType];
...
var GLTFTextureTransformExtension = class {
	...
	extendTexture(texture, transform) {
		...
		if (transform.texCoord !== void 0) console.warn("THREE.GLTFLoader: Custom UV sets in \"" + this.name + "\" extension not yet supported.");
		...
		Object.defineProperties(this, {
			...
			specularMap: {
				...
				set: function(v) {
					...
					if (v) this.defines.USE_SPECULARMAP = "";
...
var GLTFMaterialsPbrSpecularGlossinessExtension = class {
	...
	extendParams(materialParams, materialDef, parser) {
		...
		if (pbrSpecularGlossiness.diffuseTexture !== void 0) pending.push(parser.assignTexture(materialParams, "map", pbrSpecularGlossiness.diffuseTexture));
...
var GLTFParser = class {
	constructor(json = {}, options = {}) {
		...
		this.nodeNamesUsed = {};
		if (typeof createImageBitmap !== "undefined" && /Firefox/.test(navigator.userAgent) === false) this.textureLoader = new ImageBitmapLoader(this.options.manager);
		else this.textureLoader = new TextureLoader(this.options.manager);
	...
	*/
	loadAccessor(accessorIndex) {
		...
		if (accessorDef.bufferView === void 0 && accessorDef.sparse === void 0) return Promise.resolve(null);
	...
	*/
	...
	loadTextureImage(textureIndex, source, loader) {
		...
		if (source.bufferView !== void 0) sourceURI = parser.getDependency("bufferView", source.bufferView).then(function(bufferView) {
		...
		else if (source.uri === void 0) throw new Error("THREE.GLTFLoader: Image " + textureIndex + " is missing URI and bufferView");
		return Promise.resolve(sourceURI).then(function(sourceURI) {
			return new Promise(function(resolve, reject) {
				...
				loader.load(resolveURL(sourceURI, options.path), onLoad, void 0, reject);
			});
	...
	*/
	assignTexture(materialParams, mapName, mapDef) {
		...
		return this.getDependency("texture", mapDef.index).then(function(texture) {
			if (mapDef.texCoord !== void 0 && mapDef.texCoord != 0 && !(mapName === "aoMap" && mapDef.texCoord == 1)) console.warn("THREE.GLTFLoader: Custom UV set " + mapDef.texCoord + " for texture " + mapName + " not yet supported.");
	...
	*/
	assignFinalMaterial(mesh) {
		...
		if (material.aoMap && geometry.attributes.uv2 === void 0 && geometry.attributes.uv !== void 0) geometry.setAttribute("uv2", geometry.attributes.uv);
	...
	*/
	loadMaterial(materialIndex) {
		...
		if (materialDef.normalTexture !== void 0 && materialType !== MeshBasicMaterial) {
			...
			if (materialDef.normalTexture.scale !== void 0) materialParams.normalScale.set(materialDef.normalTexture.scale, -materialDef.normalTexture.scale);
		}
		...
		if (materialDef.emissiveFactor !== void 0 && materialType !== MeshBasicMaterial) materialParams.emissive = new Color().fromArray(materialDef.emissiveFactor);
		if (materialDef.emissiveTexture !== void 0 && materialType !== MeshBasicMaterial) pending.push(parser.assignTexture(materialParams, "emissiveMap", materialDef.emissiveTexture));
	...
	*/
	loadSkin(skinIndex) {
		...
		if (skinDef.inverseBindMatrices === void 0) return Promise.resolve(skinEntry);
			...
			node.traverse(function(mesh) {
				...
				for (let j = 0, jl = jointNodes.length; j < jl; j++) {
					...
					if (jointNode) {
						...
						if (skinEntry.inverseBindMatrices !== void 0) mat.fromArray(skinEntry.inverseBindMatrices.array, j * 16);
