...
// ─────────────────────────────────────────────────────
// GUARD CLAUSE — EXIT IMMEDIATELY IF CANVAS NOT ON PAGE
// ─────────────────────────────────────────────────────
...
function init3DScene() {
  ...
  function updateHeadTracking() {
    ...
    const maxPitchUp   = THREE.MathUtils.degToRad(-35);
    const maxPitchDown = THREE.MathUtils.degToRad(25);
    const maxYawLeft   = THREE.MathUtils.degToRad(-75);
    const maxYawRight  = THREE.MathUtils.degToRad(75);

    let targetPitchX = 0;
    if (mouse.y > 0) {
      targetPitchX = mouse.y * maxPitchUp;
    } else {
      targetPitchX = mouse.y * Math.abs(maxPitchDown);
    }

    let targetYawY = mouse.x * maxYawLeft;

    targetHeadEuler.set(targetPitchX, targetYawY, 0, 'XYZ');
    targetHeadQuaternion.setFromEuler(targetHeadEuler);
    headBone.quaternion.slerp(targetHeadQuaternion, 0.08);
  }
